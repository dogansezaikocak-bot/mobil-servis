...
function aiPrompt(groupRules){const existing=[...new Set(data.map(x=>x.district).filter(Boolean))];return `Bu görseller malzeme dağıtım listeleridir. Tablodaki HER satırı sırayla oku. Yalnızca JSON döndür. Şema: {"items":[{"customer":"dükkan/müşteri adı","address":"tam adres","phone":"telefon varsa","district":"mahalle veya grup","materials":["malzeme/stok/ölçü bilgileri"],"note":"belirsiz veya ek bilgi"}]}. Aynı dükkânın ardışık satırlarında farklı malzemeler varsa tek kayıtta birleştir. Başlık ve toplam satırlarını alma. Okunamayan alanı uydurma; boş bırak ve note alanına belirt. Türkçe karakterleri koru. Mevcut grup adları: ${existing.join(', ')||'yok'}. Kullanıcının grup kuralları: ${groupRules||'yok'}. Kurallar eşleşirse district alanına ilgili grup adını yaz; eşleşmezse adresteki mahalleyi yaz.`}
...
function parseAiJson(text){const cleaned=String(text||'').replace(/^```(?:json)?/i,'').replace(/```$/,'').trim();let j;try{j=JSON.parse(cleaned)}catch(e){const a=cleaned.indexOf('{'),b=cleaned.lastIndexOf('}');if(a<0||b<a)throw new Error('AI yanıtında JSON bulunamadı.');j=JSON.parse(cleaned.slice(a,b+1))}const arr=Array.isArray(j)?j:(j.items||[]);return arr.map(normalizeItem).filter(x=>x.customer||x.address||x.materials.length)}
...
function renderAiRows(){const s=document.querySelector('#distAiResultSection'),l=document.querySelector('#distAiResultList');s.hidden=false;document.querySelector('#distAiResultCount').textContent=aiRows.length+' kayıt';l.innerHTML=aiRows.map((x,i)=>`<article><div><b>${esc(x.customer||'İsimsiz')}</b><small>${esc(x.district||'Grup yok')}</small><p>${esc(x.address||'Adres yok')}</p><div>${x.materials.map(m=>`<span>${esc(m)}</span>`).join('')}</div>${x.note?`<em>${esc(x.note)}</em>`:''}</div><button type="button" data-dist="ai-remove-row" data-index="${i}">Sil</button></article>`).join('')}
...
document.addEventListener('input',e=>{if(e.target.id==='distSearch'){filters.q=e.target.value;render()}});
