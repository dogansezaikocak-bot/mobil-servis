...
async function renderProofPanel(stopId){
 ...
 try{
  ...
  for(const slot of PROOF_SLOTS){
   const photos=await proofGetAll(stopId,slot.key);
   const thumbs=photos.map((row,i)=>{const url=URL.createObjectURL(row.blob);proofObjectUrls.push(url);return `<div class="proof-thumb"><img src="${url}" alt="${esc(slot.label)} ${i+1}"><button type="button" class="proof-thumb-delete" data-dist="proof-delete" data-record="${esc(row.id)}" title="Sil">×</button></div>`}).join('');
   const status=slot.multi?(photos.length+' fotoğraf'):(photos.length?'Fotoğraf hazır':'Fotoğraf eksik');
   parts.push(`<div class="proof-slot ${photos.length?'has-photo':''}"><div class="proof-slot-previews">${thumbs||'<div class="proof-slot-preview"><span>📷</span></div>'}</div><div class="proof-slot-main"><b>${esc(slot.label)}</b><small>${status}${slot.multi?' · Birden fazla eklenebilir':''}</small><div><label class="secondary-button proof-camera">${slot.multi?'Fotoğraf Ekle':(photos.length?'Tekrar Çek':'Fotoğraf Çek')}<input type="file" accept="image/*" capture="environment" ${slot.multi?'multiple':''} data-dist-photo="${slot.key}" data-id="${stopId}"></label></div></div></div>`);
  }
