...
async function proofPut(stopId,slot,blob,multi=false){const db=await openProofDb();await new Promise((resolve,reject)=>{const tx=db.transaction(PROOF_STORE,'readwrite'),store=tx.objectStore(PROOF_STORE);if(!multi){const req=store.getAll();req.onsuccess=()=>{for(const row of req.result||[])if(row.stopId===stopId&&row.slot===slot)store.delete(row.id);store.put({id:proofRecordId(stopId,slot),stopId,slot,blob,updatedAt:Date.now()})};req.onerror=()=>reject(req.error)}else store.put({id:proofRecordId(stopId,slot),stopId,slot,blob,updatedAt:Date.now()});tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});db.close();await refreshProofCounts()}
async function proofDeleteRecord(recordId){const db=await openProofDb();await new Promise((resolve,reject)=>{const tx=db.transaction(PROOF_STORE,'readwrite');tx.objectStore(PROOF_STORE).delete(recordId);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});db.close();await refreshProofCounts()}
...
async function refreshProofCounts(){try{const rows=await proofAll();proofCounts=new Map();for(const row of rows){const s=proofCounts.get(row.stopId)||{total:0,slots:new Set()};s.total++;s.slots.add(row.slot);proofCounts.set(row.stopId,s)}}catch(e){console.warn(e)}}
...
async function renderProofPanel(stopId){const box=document.querySelector(`[data-proof-panel="${stopId}"]`);if(!box)return;box.innerHTML='<p class="proof-loading">Fotoğraflar yükleniyor…</p>';const parts=[];for(const slot of PROOF_SLOTS){const photos=await proofGetAll(stopId,slot.key);const thumbs=photos.map((row,i)=>{const url=URL.createObjectURL(row.blob);proofObjectUrls.push(url);return `<div class="proof-thumb"><img src="${url}" alt="${esc(slot.label)} ${i+1}"><button type="button" class="proof-thumb-delete" data-dist="proof-delete" data-record="${esc(row.id)}" title="Sil">×</button></div>`}).join('');const status=slot.multi?(photos.length+ ' fotoğraf'):(photos.length?'Fotoğraf hazır':'Fotoğraf eksik');parts.push(`<div class="proof-slot ${photos.length?'has-photo':''}"><div class="proof-slot-previews">${thumbs||'<div class="proof-slot-preview"><span>📷</span></div>'}</div><div class="proof-slot-main"><b>${esc(slot.label)}</b><small>${status}${slot.multi?' · Birden fazla eklenebilir':''}</small><div><label class="secondary-button proof-camera">${slot.multi?'Fotoğraf Ekle':(photos.length?'Tekrar Çek':'Fotoğraf Çek')}<input type="file" accept="image/*" capture="environment" ${slot.multi?'multiple':''} data-dist-photo="${slot.key}" data-id="${stopId}"></label></div></div></div>`)}box.innerHTML=parts.join('')}
async function shareProofPhotos(stopId){const stop=data.find(x=>x.id===stopId);if(!stop)return;const files=[];let sequence=1;for(const slot of PROOF_SLOTS){const rows=await proofGetAll(stopId,slot.key);if(!rows.length){alert(`${slot.label} fotoğrafı eksik.`);return}for(let i=0;i<rows.length;i++){const suffix=rows.length>1?'_'+String(i+1).padStart(2,'0'):'';files.push(new File([rows[i].blob],`${safeFileName(stop.customer)}_${slot.file}${suffix}.jpg`,{type:'image/jpeg'}));sequence++}}try{if(navigator.share&&(!navigator.canShare||navigator.canShare({files}))){await navigator.share({files,title:stop.customer,text:`${stop.customer}
...
async function photoFileToJpeg(file){let source=file;if(isHeicFile(file))source=await heicToJpegBlob(file);const img=await loadImageFromBlob(source),max=1800,scale=Math.min(1,max/Math.max(img.naturalWidth,img.naturalHeight)),c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.naturalWidth*scale));c.height=Math.max(1,Math.round(img.naturalHeight*scale));const ctx=c.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(img,0,0,c.width,c.height);return await new Promise((resolve,reject)=>c.toBlob(b=>b?resolve(b):reject(new Error('Fotoğraf hazırlanamadı.')),'image/jpeg',0.86))}
function revokeProofUrls(){for(const u of proofObjectUrls)URL.revokeObjectURL(u);proofObjectUrls=[]}
async function renderProofPanel(stopId){const box=document.querySelector(`[data-proof-panel="${stopId}"]`);if(!box)return;box.innerHTML='<p class="proof-loading">Fotoğraflar yükleniyor…</p>';const rows=[];for(const slot of PROOF_SLOTS){const row=await proofGet(stopId,slot.key);let preview='';if(row?.blob){const url=URL.createObjectURL(row.blob);proofObjectUrls.push(url);preview=`<img src="${url}" alt="${esc(slot.label)}">`}rows.push(`<div class="proof-slot ${row?'has-photo':''}"><div class="proof-slot-preview">${preview||'<span>📷</span>'}</div><div class="proof-slot-main"><b>${esc(slot.label)}</b><small>${row?'Fotoğraf hazır':'Fotoğraf eksik'}</small><div><label class="secondary-button proof-camera">${row?'Tekrar Çek':'Fotoğraf Çek'}<input type="file" accept="image/*" capture="environment" data-dist-photo="${slot.key}" data-id="${stopId}"></label>${row?`<button class="danger-button proof-delete" data-dist="proof-delete" data-id="${stopId}" data-slot="${slot.key}">Sil</button>`:''}</div></div></div>`)}box.innerHTML=rows.join('')}
async function shareProofPhotos(stopId){const stop=data.find(x=>x.id===stopId);if(!stop)return;const files=[];for(const slot of PROOF_SLOTS){const row=await proofGet(stopId,slot.key);if(row?.blob)files.push(new File([row.blob],`${safeFileName(stop.customer)}_${slot.file}.jpg`,{type:'image/jpeg'}))}if(files.length<5){alert(`Bu kartta ${files.length}/5 fotoğraf var. Önce eksik fotoğrafları çek.`);return}try{if(navigator.share&&(!navigator.canShare||navigator.canShare({files}))){await navigator.share({files,title:stop.customer,text:`${stop.customer}\n${stop.address}`});return}}catch(e){if(e?.name==='AbortError')return;console.warn(e)}for(const file of files){const a=document.createElement('a');a.href=URL.createObjectURL(file);a.download=file.name;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),3000)}alert('Telefon çoklu paylaşımı desteklemedi. 5 fotoğraf indirildi.')}
...
function render(){
 ...
 <div class="dist-head"><div><span class="section-kicker">V8.2.1 Çoklu Dolap Fotoğrafları</span><h2>Malzeme Operasyon Sistemi</h2><p>Dükkân ve servis fişi için tek; montaj öncesi, montaj sonrası ve seri/model için istediğin kadar fotoğraf kaydet ve topluca paylaş.</p></div><div class="dist-head-actions"><button class="secondary-button dist-ai-main-button" data-dist="ai-open">📄 AI Listeyi Oku</button><button class="secondary-button" data-dist="import">Liste İçe Aktar</button><button class="primary-button" data-dist="add">+ Yeni Durak</button></div></div>
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-dist]');if(!b)return;const a=b.dataset.dist,id=b.dataset.id;
 ...
 if(a==='delete-selected'){if(!selectedStops.size)return;if(confirm(selectedStops.size+' seçili dağıtım silinsin mi?')){const ids=[...selectedStops];data=data.filter(x=>!selectedStops.has(x.id));selectedStops.clear();ids.forEach(proofDeleteStop);save();render()}return}
 if(a==='delete-all'){if(confirm('Bütün dağıtım listesi ve fotoğrafları silinsin mi? Bu işlem geri alınamaz.')){const ids=data.map(x=>x.id);data=[];selectedStops.clear();ids.forEach(proofDeleteStop);save();render()}return}
