...
document.addEventListener('submit',e=>{if(e.target.id!=='distributionForm')return;e.preventDefault();e.stopPropagation();const f=e.target;const old=data.find(x=>x.id===f.elements.id.value);const parsed=f.elements.materials.value.split(/\n+/).filter(Boolean).map(line=>{const p=line.split('|').map(v=>v.trim());return {cooler:p[0]||'',new_design:p[1]||'',quantity:p[2]||1}});const item=normalizeItem({id:f.elements.id.value,customer:f.elements.customer.value,district:f.elements.district.value,address:f.elements.address.value,phone:f.elements.phone.value,materials:parsed,status:f.elements.status.value,note:f.elements.note.value});if(old&&f.elements.status.value!=='delivered'){const oldMap=new Map((old.materials||[]).map(m=>[materialKey(m),m]));for(const m of item.materials){const om=oldMap.get(materialKey(m));if(om)m.delivered=Boolean(om.delivered)}}syncStatus(item);const i=data.findIndex(x=>x.id===item.id);if(i>=0)data[i]=item;else data.push(item);data=mergeDuplicates(data);save();document.querySelector('#distributionDialog').close();render();alert('Değişiklikler kaydedildi.');});
