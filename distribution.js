...
let cloudBusy=false,cloudStatus='',cloudTimer=null,cloudPhotoIndex=new Map();
...
function save(){saveLocalOnly();cloudPush()}
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-dist]');if(!b)return;const a=b.dataset.dist,id=b.dataset.id;
 ...
 if(a==='cloud-settings'){openCloudSettings();return}if(a==='cloud-refresh'){cloudPull();return}if(a==='delete-selected'){if(!selectedStops.size)return;if(confirm(selectedStops.size+' seçili dağıtım silinsin mi?')){const ids=[...selectedStops];data=data.filter(x=>!selectedStops.has(x.id));selectedStops.clear();ids.forEach(proofDeleteStop);save();render()}return}
