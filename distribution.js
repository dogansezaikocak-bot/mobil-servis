...
function normalizeMaterial(m){
 if(typeof m==='string')return {id:uid(),name:m.trim(),cooler:m.trim(),newDesign:'',quantity:1,delivered:false};
 ...
 const name=String(m.name||[cooler,newDesign].filter(Boolean).join(' · ')).trim();
 return {id:m.id||uid(),name,cooler:cooler||name,newDesign,quantity,delivered:Boolean(m.delivered)};
}
function materialKey(m){return foldTr((m.cooler||m.name||'')+'|'+(m.newDesign||''))}
function materialLabel(m){return [m.cooler||m.name,m.newDesign].filter(Boolean).join(' · ')}
function groupedMaterials(stop){
 const map=new Map();
 for(const m of stop.materials||[]){const k=materialKey(m);const cur=map.get(k)||{key:k,cooler:m.cooler||m.name,newDesign:m.newDesign||'',quantity:0,deliveredQuantity:0,ids:[]};cur.quantity+=Math.max(1,Number(m.quantity)||1);if(m.delivered)cur.deliveredQuantity+=Math.max(1,Number(m.quantity)||1);cur.ids.push(m.id);map.set(k,cur)}
 return [...map.values()];
}
...
document.addEventListener('submit',e=>{if(e.target.id!=='distributionForm')return;e.preventDefault();e.stopImmediatePropagation();const f=e.target;const old=data.find(x=>x.id===f.elements.id.value);const selectedStatus=f.elements.status.value;const parsed=f.elements.materials.value.split(/\n+/).filter(Boolean).map(line=>{const p=line.split('|').map(v=>v.trim());return {cooler:p[0]||'',new_design:p[1]||'',quantity:p[2]||1}});const item=normalizeItem({id:f.elements.id.value,customer:f.elements.customer.value,district:f.elements.district.value,address:f.elements.address.value,phone:f.elements.phone.value,materials:parsed,status:selectedStatus,note:f.elements.note.value});if(selectedStatus==='delivered'){item.materials.forEach(m=>m.delivered=true);item.status='delivered';item.deliveredAt=old?.deliveredAt||new Date().toISOString()}else{item.materials.forEach(m=>m.delivered=false);item.status='waiting';item.deliveredAt=''}const i=data.findIndex(x=>x.id===item.id);if(i>=0)data[i]=item;else data.push(item);data=mergeDuplicates(data);save();const dlg=document.querySelector('#distributionDialog');if(dlg?.open)dlg.close();render();alert(selectedStatus==='delivered'?'Teslim durumu kaydedildi.':'Kayıt beklemeye geri alındı.');});
